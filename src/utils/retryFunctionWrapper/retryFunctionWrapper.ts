...
export function retryFunctionWrapper<T extends (...args: any[]) => Promise<any>>({
    fn,
    attempts,
    delay,
    onFailAttempt,
    ...
    return ((...args: Parameters<T>): Promise<ReturnType<T>> => new Promise<ReturnType<T>>((resolve, reject) => {
        let attemptIndex = 0;
